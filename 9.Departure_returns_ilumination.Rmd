...
### Calculate pairiwise departure times for 2023
```{r}
#split data frame by id
...
ggsave(file="~/ownCloud/PhDLife/P.hastatus/Thesis/Paper3/analysis/figures/depart_moonlight_wspeed.pdf", height=10, width=10)
...
### How does departure time difference related to moon light?
```{r}
#install.packages("calendR")
library(calendR)

#chekc if its related to lunar cycles
...
# Test if departure time and moon illumination have a relationship in february data

s(illumination): A smooth term to model the non-linear relationship between moonlight intensity and departure time.
s(date_f, bs = "re"): Treats date as a random effect using the "re" (random effects) basis, which is appropriate for categorical variables.
random = list(tag_local_identifier = ~1): Specifies tag_local_identifier as a random intercept to account for repeated measures on individuals.
```{r}
library(mgcv)
deps$date_f <- as.factor(deps$date)

lme_feb_moon <- lmer(log(as.numeric(timeSinceSunset)) ~ illumination + (1|tag_local_identifier), 
                   data = deps)

summary(lme_feb_moon)
car::Anova(lme_feb_moon)

##the gam model is not positive
gamm_feb_moon <- gamm(log(as.numeric(timeSinceSunset)) ~ s(illumination) + s(date_f, bs="re"), random = list(tag_local_identifier = ~1), 
                   data = deps)

summary(gamm_feb_moon$gam)
summary(gamm_feb_moon$lme)

par(mfrow = c(2, 2))  # To see multiple plots at once
plot(gamm_feb_moon$gam)

# check for Autocorrelation in Residuals (since you have repeated measures):
acf(resid(gamm_feb_moon$lme), main = "ACF of residuals")

# Check Random Effects:
# random effects to see how much individual variability is being captured.
ranef(gamm_feb_moon$lme)  # Extract random effects

#The gam.check() function helps identify issues like concurvity (smooth terms that are too closely correlated) and whether more knots might be needed.
gam.check(gamm_feb_moon$gam)

#You can visualize the fitted smooth terms to see how they capture the relationship between illumination and timeSinceSunset, as well as the effect of date.
plot(gamm_feb_moon$gam, pages = 1)

#The smooth terms should have confidence intervals (dashed lines) that show where the model has high confidence in the relationship.
#A wiggly smooth term suggests non-linearity, while a flatter smooth indicates a linear relationship.

# Colliniarity
concurvity(gamm_feb_moon$gam, full = TRUE) #High concurvity values (close to 1) suggest multicollinearity, which might distort the model.
```
# Test if departure time and moon illumination have a relationship in march data
```{r}
commu_march_dep$date_f <- as.factor(commu_march_dep$date)

gamm_march_moon <- gamm(as.numeric(timeSinceSunset) ~ s(illumination) + s(date_f, bs="re"), random = list(tag_local_identifier = ~1), 
                   data = commu_march_dep)

summary(gamm_march_moon$gam)
summary(gamm_march_moon$lme)

par(mfrow = c(2, 2))  # To see multiple plots at once
plot(gamm_march_moon$gam)

# check for Autocorrelation in Residuals (since you have repeated measures):
acf(resid(gamm_march_moon$lme), main = "ACF of residuals")

# Check Random Effects:
# random effects to see how much individual variability is being captured.
ranef(gamm_march_moon$lme)  # Extract random effects

#The gam.check() function helps identify issues like concurvity (smooth terms that are too closely correlated) and whether more knots might be needed.
gam.check(gamm_march_moon$gam)

#You can visualize the fitted smooth terms to see how they capture the relationship between illumination and timeSinceSunset, as well as the effect of date.
plot(gamm_march_moon$gam, pages = 1)

#The smooth terms should have confidence intervals (dashed lines) that show where the model has high confidence in the relationship.
#A wiggly smooth term suggests non-linearity, while a flatter smooth indicates a linear relationship.

# Colliniarity
concurvity(gamm_march_moon$gam, full = TRUE) #High concurvity values (close to 1) suggest multicollinearity, which might distort the model.
```